	...
	: BaseClass( parent, panelName )
{
	...
	//
	if ( !V_strcmp( g_engineDir, "..\\bin" ) )
	   m_pModHL2Button->SetEnabled( IsGameSubscribed( GetAppSteamAppId( k_App_SteamPipe_BASE_SP ) ) );
	else
	{
		// do it without nesting.
		if ( ( IsGameSubscribed( GetAppSteamAppId( k_App_SteamPipe_BASE_MP ) ) ) )  
			m_pModHL2Button->SetEnabled( true );
		else
			m_pModHL2Button->SetEnabled( false );
	}

	m_pModHL2MPButton->SetEnabled( IsGameSubscribed( GetAppSteamAppId( k_App_SteamPipe_BASE_MP ) ) );
